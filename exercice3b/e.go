...
func process(text string) string {
	...
	priorities := 0
	groupsCount := len(lines) / 3

	for idx := 0; idx < groupsCount; idx += 1 {
		mainBag := lines[idx*3]
		subBags := []string{lines[idx*3+1], lines[idx*3+2]}

		for idx := 0; idx < len(mainBag); idx += 1 {
			char := mainBag[idx]
			if IsInBag(char, subBags[0]) && IsInBag(char, subBags[1]) {
				priorities += ByteToScore(char)
				break
			}
		}
	}

	return fmt.Sprintf("%d", priorities)
}

func IsInBag(b byte, bag string) bool {
	for idx := 0; idx < len(bag); idx += 1 {
		if b == bag[idx] {
			return true
		}
	}
...
var input = `gfWpjRRQffQGCHHJsGqjsj
